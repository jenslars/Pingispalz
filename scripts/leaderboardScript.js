...
function sortPlayersByWins(playerData) {
  ...
  updateLeaderboard(playerData, pendingMatches);
}
...
function sortPlayersByElo(playerData) {
  playerData.sort((a, b) => b.elo - a.elo);
  updateLeaderboard(playerData, pendingMatches);
}

elo.addEventListener("click", function() {
  sortPlayersByElo(playerData);    
})
...
function updateLeaderboard(playerData, data) {
  ...
  playerData.forEach(player => {
    ...
    // Check if the player has a pending match
    const hasPendingMatch = data.some(match =>
      match.recipient_id === player.user_id && match.status === "PENDING"
    );
